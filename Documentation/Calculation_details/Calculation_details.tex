...
\title{\textbf{\LARGE Modélisation de lentilles électrostatiques quadrupolaires à l'aide de la méthode des éléments de frontière}
\\[0.5em]\LARGE Développement des équations nécessaires et de la stratégie de calcul}
\author{Zoé Nonon, Louise Lammertyn}
\date{\textit{Année 2025-2026}}
...
\]
\\
Dans le cas de l'optique des particules chargées, l'indice optique s'écrit 
\[
\mu = \sqrt{\phi(1+(x')^2 + (y')^2)}
\]
On obtient ainsi le lagrangien du système 
\[
...
\subsection{Cas $\nu$ = 2, le quadrupole}
Si on prend le cas $\nu$ = 2, le potentiel devient 

\[
\phi_2(r,\theta,z) = r^{2}(\phi_2(z) \cos(2\theta)) + \frac{r^{4}}{12} (\phi_2''(z) \cos(4\theta))
\]
\\
avec $r^2\cos(2\theta)=x^2-y^2$
\\
On obtient ainsi dans l'approximation paraxiale 
\[
\phi_2(r, \theta, z) =\phi_2(z)(x^2 -y^2)
\]
\\
et dans le cas paraxial 
...
\\
\\
On peut ensuite injecter ce potentiel dans l'indice optique 
\[
\mu = \sqrt{\phi(1+(x')^2 + (y')^2)}
\]
\\
\\
On peut alors retrouver, après développement, les équations d'Euler-Lagrange, dans le cas d'un quadrupole
On peut écrire l'indice optique comme 
\[
\mu = \sqrt{\phi(1+\epsilon)}
\]
\\
 avec $\epsilon : (x')^2 +(y')^2$.
\\
A l'aide d'un développement de Taylor, on a $\sqrt{1+\epsilon} = 1 +\frac{\epsilon}{2}$ \\
\\
...
{\color{red}PAS SURE DE L'EQUATION PRECEDENTE}
...
On peut injecter $\mu$ dans les équations d'Euler-Lagrange pour retrouver 
...
Il nous reste donc à resoudre les équations différentielles suivantes à l'aide de la méthode Runge-Kutta 4 afin d'obtenir les trajectoires des ions
...
On peut trouver les solutions telles que:
\\
 Rayon principal : $x(z_0) = 1, x'(z_0) = 0$
\\
 Rayon marginal : $ x(z_0) = 0, x'(z_0) = 1$

\subsection{Résolution des équations différentielles}
{\color{blue}Cas sans polarisation des apertures.}
\\Dans les deux cas à résoudre, on a une équation différentielle de type 
\[
x''(z) + \omega_0^2 x = 0
\]
On résout cette équation avec la methode Runge Kutta 4 avec les condtions initales suivantes:\\
    Pour le rayon principal : $x(z_0) = 1$ et $x'(z_0) = 0$\\
    Pour le rayon marginal : $x(z_0) = 0$ et  $x'(z_0) = 1$ 
\\

\subsection{Vérification des résultats}
\subsubsection{Cas $\omega_0 > 0$}

On verifie les résultats analityquement.\\
Dans le cas où $\omega_0^2 > 0$, on a, où on a une focalisation selon la direction x:
\[
x''(z) + \omega_0^2 x(z) = 0 .
\]
...
On obtient finalement la solution analytique du rayon marginal telle que
\[
\boxed{
...
On obtient finalement la solution pour le rayon principal,
\[
\boxed{
...
\subsubsection{Cas $\omega_0 <0$}
Dans le cas où $\omega_0$ est inférieur à 0, et donc dans la direction y, on obtient la trajectoire de défocalisation. On a 
\[
y''(z) + \omega_0^2 y(z) = 0 
\]
