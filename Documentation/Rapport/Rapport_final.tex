...
\begin{titlepage}
    ...
    {\large
    \begin{tabular}{l}
        Florent \textsc{Houdellier} - Insa Toulouse \\
        Lucile \textsc{Berdoux Durieux} - Tescan \\
        Jean-Baptiste \textsc{Mellier} - Tescan\\
        Pier-Francesco \textsc{Fazzini} - Insa Toulouse
    \end{tabular}
    } \par
...
\section{Principe de correction d'aberration}
L'utilisation de particules chargées réduit l'impact de la diffraction. 
Cependant, la résolution  des instruments reste limitée par des aberrations. 
On distingue principalement deux types d’aberrations :
\begin{itemize}
    \item les aberrations chromatiques, liées à la dispersion en énergie des particules,
    \item les aberrations géométriques.
\end{itemize}

Parmi les aberrations géométriques, on trouve notamment :
\begin{itemize}
    \item l’aberration sphérique,
    \item le coma,
    \item la courbure de champ,
    \item l’astigmatisme,
    \item la distorsion.
\end{itemize}

Ce rapport se concentre sur l’aberration sphérique.

L’aberration sphérique traduit le fait que les particules passant loin de l’axe optique sont focalisées différemment de celles proches de l’axe. Il en résulte un élargissement du point focal, ce qui limite directement la résolution de l’instrument.

La correction de cette aberration est donc essentielle pour atteindre des performances élevées en optique des particules chargées.



 Ce rapport se concentrera à l'abbération shérique. 
 L'abbération spérique traduit le fait les particules passant loin de l’axe sont focalisées
  différemment de celles proches de l’axe. La correction
de ces aberrations est essentielle pour améliorer la
résolution d'instruments.

\begin{figure}[H]
\centering
\includegraphics[width=0.3\linewidth]{aberation_spherique.png}
\caption{abérration sphérique }
\label{fig:abberation}
\end{figure}

la méthode utilisé pour corrigr l'abbération sphérique est l'uilisation de correcteur multipolaire. 
La supersposition des systeme octopolaire et octopolaire permet la reductiobn de l'abbération sphérique. 
Cependant un defi vient sur l'aligment des different étages. 
...
multipolaire d'un quadrupole électrostatique.  
