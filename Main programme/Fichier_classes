...
class Mesh_Generation: 
    ...
    def Geometry(self):
        ...
        cylinder1 = Cylinder(self.length_cylinder, self.radius_axis, self.dimensions.coord_cylinder_x_or_y, 0, self.dimensions.coord_cylinder_z)
        Cylinder.ajouter(cylinder1)

        cylinder2 = Cylinder(self.length_cylinder, self.radius_axis, -self.dimensions.coord_cylinder_x_or_y, 0, self.dimensions.coord_cylinder_z)
        Cylinder.ajouter(cylinder2)

        cylinder3 = Cylinder(self.length_cylinder, self.radius_axis, 0, self.dimensions.coord_cylinder_x_or_y, self.dimensions.coord_cylinder_z)
        Cylinder.ajouter(cylinder3)

        cylinder4 = Cylinder(self.length_cylinder, self.radius_axis, 0, -self.dimensions.coord_cylinder_x_or_y, self.dimensions.coord_cylinder_z)
        Cylinder.ajouter(cylinder4)


        aperture1 = Aperture(self.radius_apert, self.radius_axis, self.thickness_apert, 0, 0, self.dimensions.coord_apert_z1)
        Aperture.ajouter(aperture1)

        aperture2 = Aperture(self.radius_apert, self.radius_axis, self.thickness_apert, 0, 0, self.dimensions.coord_apert_z2)
    ...
    #Outwards orientation of the surfaces' normals
        ...
        self.group_id = group_id_apert1, group_id_apert2, group_id_cyl1, group_id_cyl2, group_id_cyl3, group_id_cyl4, group_id_shield
    ...
    def Finalize(self):
    #Creates a file .msh
        mesh_path = os.path.join(self.output_dir, "mesh_quadrupole.msh")
...
class Calculation_field:
    def __init__(self, elec_id, pot_apert1, pot_apert2, pot_electrode, pot_shield, pot_acceleration,dimensions, output_dir):
        ...
        self.mesh_generation = mesh_generation
        self.output_dir = output_dir

        #Importation of the mesh
        self.mesh_path = os.path.join(self.output_dir, "mesh_quadrupole.msh")
    ...
    #Resolution of the linear system
        ...
        n_grid_points = 100
        self.points = np.stack((np.zeros(n_grid_points), np.zeros(n_grid_points), (np.linspace(0, dimensions.total_length, n_grid_points)))) #x=y=0 and z changes from 0 to elec_id[7][11]=total_length
    ...
    def Potential_exportation(self):
        ...
        savefile = os.path.join(self.output_dir, "potentiel_quadrupole_VF.npz")
        ...
        self.dimensions.radius_apert, self.dimensions.thickness_apert, self.dimensions.length_cylinder, self.dimensions.radius_axis, self.dimensions.total_length, self.dimensions.coord_cylinder_x_or_y, 
        self.dimensions.coord_cylinder_z, self.dimensions.coord_apert_z1, self.dimensions.coord_apert_z2)),
...
class Fit_constants:
    def __init__(self, a0, b0, b2, a4, b41, b42, z0, a, data):
    ...
    def fonction_fit(self):

        dist_apert_quad = self.data.dimension[1]
        thickness_apert = self.data.dimension[6]
        length_cylinder = self.data.dimension[7]
        thickness_shield = self.data.dimension[4]
        dist_shield_apert = self.data.dimension[0]
...
class Decomposition:
    def __init__(self, potentials, data):
        self.data = data 
    ...
    def composantes(self):
        ...
        self.Phi0_fit = self.Phi0_maj / self.potentials.pot_apert1
        self.Phi2_fit = self.Phi2_maj *((dimensions.radius_axis**2)/ self.potentials.pot_electrode)
        self.Phi4_fit = self.Phi4_maj *((dimensions.radius_axis**4)/ self.potentials.pot_apert1)
...
class Affichage:
    def __init__(self, data, dimensions, decomposition, fit):
        self.data = data
        self.dimensions = dimensions
